...
## ~~P1-5 ask + agent 合并成「助理」~~ —— 不做（产品负责人 2026-08-14 决定）

P0 落地后产品负责人决定跳过这一条。保留原设计于下方而不是删掉，理由和 review 里那两条划掉项一样：
这是一个被**考虑过并主动否决**的方向，不是没想到——将来谁再提「为什么不把 ask 和 agent 合并」，
应该能读到当时的设计和它被搁置的时点，而不是从头再推一遍。
