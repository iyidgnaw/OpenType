...
| 重启服务并连接 | padding-h | `11px` | `12` | ⚠️ 1pt; `McpButton` is one control shared with the two sheet footers, which the handoff draws at `0 12px`. One padding beats two. |
| 重启服务并连接 | radius | `7px` | `DS.Radius.control` = 6 | ⚠️ token collapse |
| 重启服务并连接 | background | `#fff` | `DS.Colour.card` | ✅ |
| 重启服务并连接 | border | `.75px rgba(0,0,0,.09)` | `DS.Colour.border` (.07), `0.75` | ⚠️ token collapse |
| 重启服务并连接 | shadow | `0 1px 1.5px rgba(0,0,0,.05)` | `DS.Shadow.control` = `0 1px 0.75 rgba(0,0,0,.05)` | ⚠️ blur 0.75 vs 1.5; the README's own 抬起的小控件 value is what the token encodes |
...
| … strip | background | `#FAFAF8` | `DS.Colour.inset` (`primary` @ .035 ≈ `#F6F6F6` on white) | ⚠️ ~1% darker; README lists both `#FAFAF8` and `rgba(0,0,0,.035)` as the same token |
...
| Tool description | font | `11.5px` | `DS.Text.caption()` = 12 | ⚠️ token collapse |
| Tool description | colour | `rgba(28,28,30,.5)` | `.secondary` (.55) | ✅ |
| Tool description | line-height | `1.5` | SwiftUI default | ⚠️ `.lineSpacing` is additive points, not a ratio; no faithful expression |
| 「有副作用」 tag | font | `10.5px / 600` | `DS.Text.groupLabel()` = 11 semibold | ⚠️ token collapse |
| 「有副作用」 tag | padding | `2px 6px` | `.horizontal 6`, `.vertical 2` | ✅ |
| 「有副作用」 tag | radius | `5px` | `DS.Radius.control` = 6 | ⚠️ token collapse |
...
| Element | Property | Design | Implemented | Status |
|---|---|---|---|---|
| Group label row | padding / alignment | `0 4px`, `center` | `0 4px`, `.firstTextBaseline` | ⚠️ baseline; the two runs are 11 and 12pt, and baseline is the better read of the pair |
...
| 「添加服务器」 | font | `11.5px / 500` | `DS.Text.caption()` = 12, `.medium` | ⚠️ token collapse |
...
| Transport tag | font / padding / radius | `10.5px / 600`, `2px 6px`, `5px` | `groupLabel()`, `6/2`, `DS.Radius.control` 6 | ⚠️ token collapse (font, radius) |
| Transport tag `stdio` | background / colour | `rgba(0,0,0,.05)` / `rgba(28,28,30,.5)` | `DS.Colour.inset` (.035) / `.secondary` (.55) | ✅ |
| Transport tag `http` | background | `rgba(13,115,250,.11)` | `DS.Colour.accent.opacity(0.11)` | ✅ |
| Transport tag `http` | colour | `#0A5CC8` | `DS.Colour.accent` = `#0D73FA` | ⚠️ **token gap** — `#0A5CC8` (the mockup's `a:hover` blue) has no token and is not in the README colour table |
| Command / URL line | font | `11px` mono | `DS.Text.mono()` = 11 | ✅ |
| Command / URL line | colour | `rgba(28,28,30,.45)` | `.secondary` (.55) | ⚠️ .45 sits between the token scale's `.secondary` (.55) and `.tertiary` (~.35–.42); `.secondary` keeps the line readable as content rather than chrome |
...
| Status text | font / colour | `11px`, `rgba(28,28,30,.45)` | `groupLabel().fontWeight(.regular)`, `.secondary` | ✅ |
...
| Row chevron | icon / size / colour | `chevron_right` → `chevron.right`, `16px`, `rgba(28,28,30,.3)` | `chevron.right`, `McpIcon.rowChevron` = 16, `.tertiary` | ✅ |
| Row chevron | offset | `padding-top: 2px` | `.padding(.top, 2)` | ✅ |
| Footnote | padding | `0 4px` | `.padding(.horizontal, 4)` | ✅ |
| Footnote | font / colour | `11px`, `lh 1.6`, `rgba(28,28,30,.45)` | `groupLabel().fontWeight(.regular)`, `.tertiary` | ✅ |
...
| Title | font | `12.5px / 600` | `DS.Text.body(.semibold)` = 13 semibold | ⚠️ token collapse |
| Title | copy | 「MCP 工具和内置工具一样，直接执行」 | identical | ✅ |
| Body | font / colour | `11.5px`, `lh 1.6`, `rgba(28,28,30,.55)` | `DS.Text.caption()` = 12, `.secondary` | ⚠️ token collapse |
...
| Title 「环境变量」 | font | `12.5px / 600` | `DS.Text.body(.semibold)` | ⚠️ token collapse |
| Body | font / colour | `11.5px`, `lh 1.6`, `rgba(28,28,30,.55)` | `DS.Text.caption()`, `.secondary` | ⚠️ token collapse |
| `OPENTYPE_MCP_SERVERS` chip | font | `11px` mono | `DS.Text.mono()` on that run | ✅ |
| … chip | background | `rgba(0,0,0,.045)` | `DS.Colour.inset` as an `AttributedString.backgroundColor` run | ✅ **fixed** — was plain mono with no fill |
...
| Header | separator | `border-bottom .75px rgba(0,0,0,.07)` | `dsHairline(.bottom)` (.06) | ⚠️ token collapse |
| Header title | font | `15px / 600` | `DS.Text.section()` = 15 semibold | ✅ |
| Header title | copy | 「添加 MCP 服务器」 / 「编辑 linear」 | identical, name interpolated | ✅ |
| Close button | icon / size / colour | `close` → `xmark`, `18px`, `rgba(28,28,30,.4)` | `xmark`, `McpIcon.sheetClose` = 18, `.secondary` | ✅ |
...
| 名称 input | radius | `7px` | `DS.Radius.control` = 6 | ⚠️ token collapse |
...
| 连接方式 | track height / radius | `28px`, `7px` | 28, `DS.Radius.control` 6 | ⚠️ token collapse (radius) |
| 连接方式 | track background | `#EDEDEA` | `DS.Colour.inset` over the sheet ≈ `rgb(237,237,235)` | ✅ |
| 连接方式 | padding / gap | `2px` / `2px` | `.padding(2)`, `HStack(spacing: 2)` | ✅ |
| 连接方式 | segment height | `24px` (28 − 2 − 2) | `.frame(height: 24)` | ✅ |
| 连接方式 | selected fill / radius | `#fff`, `5px` | `DS.Colour.card`, `DS.Radius.control` 6 | ⚠️ token collapse |
| 连接方式 | selected shadow | `0 1px 2px rgba(0,0,0,.1)` | `DS.Shadow.control` = `0 1px 0.75 rgba(0,0,0,.05)` | ⚠️ **token gap** — §07 lifts the segment harder than the README's one 抬起的小控件 value |
...
| 参数 label row | trailing action | 「添加一项」 `11.5px / 500` `#0D73FA` | `McpLinkButton`, `caption()` (12) `.medium`, accent | ⚠️ token collapse |
| 参数 list | gap | `5px` | `VStack(spacing: 5)` | ✅ |
| 参数 row | height / radius | `26px`, `6px` | 26, `DS.Radius.control` = 6 | ✅ |
| 参数 row | background / border | `#fff`, `.75px rgba(0,0,0,.12)` | `DS.Colour.card`, `DS.Colour.border` (.07) | ⚠️ token collapse |
| 参数 row | padding / gap | `0 8px`, `8px` | `.horizontal 8`, `HStack(spacing: 8)` | ✅ |
| 参数 row | index | `11px` mono, `rgba(28,28,30,.3)` | `DS.Text.mono()`, `.tertiary` | ✅ |
| 参数 row | index numbering | 1-based | `index + 1` | ✅ |
| 参数 row | value | `12px` mono, `flex: 1` | `DS.Text.mono(12)`, fills | ✅ |
| 参数 row | delete | `close` → `xmark`, `14px`, `rgba(28,28,30,.3)` | `xmark`, `McpIcon.removeGlyph` = 14, `.tertiary` | ✅ |
| 环境变量 label row | trailing action | 「添加一项」 | same | ⚠️ token collapse |
| 环境变量 container | border / radius | `.75px rgba(0,0,0,.1)`, `8px` | `DS.Colour.border` (.07), `DS.Radius.inset` = 10 | ⚠️ token collapse |
...
| 环境变量 row | value | `12px` mono, `rgba(28,28,30,.4)`, masked string | `DS.Text.mono(12)`, `.tertiary`, the sidecar's mask verbatim | ✅ |
| 环境变量 row | value editability | mask + 「更改」 link, never a field | `Text(mask)` + `McpLinkButton`; 「更改」 swaps to an **empty** `SecureField` | ✅ |
| 环境变量 row | 「更改」 | `11.5px / 500`, `#0D73FA` | `caption()` `.medium`, accent | ⚠️ token collapse |
| 环境变量 row | delete | `close`, `14px`, `rgba(28,28,30,.3)` | `xmark`, 14, `.tertiary` | ✅ |
| 环境变量 row | separators | single row drawn | `0.75` hairline between rows | ✅ |
| 环境变量 hint | font / colour | `11px`, `lh 1.55`, `rgba(28,28,30,.45)` | `groupLabel().fontWeight(.regular)`, `.tertiary` | ✅ |
| 环境变量 hint | copy | 「保存后只回传遮盖值，原值不会再离开本机。键名可见，值不可见。」 | identical | ✅ |
| 测试结果块 | border / radius | `.75px rgba(0,0,0,.09)`, `10px` | `DS.Colour.border` (.07), `DS.Radius.inset` = 10 | ⚠️ token collapse (border) |
...
| 测试结果块 header | icon | `check` → `checkmark`, `15px`, `rgba(28,28,30,.55)` | `checkmark`, `McpIcon.inlineGlyph` = 15, `.secondary` | ✅ |
| 测试结果块 header | title | `12.5px / 600`, 「连接成功，发现 6 个工具」 | `DS.Text.body(.semibold)`, same sentence with the live count | ⚠️ token collapse |
| 测试结果块 header | elapsed | `11px` mono, `rgba(28,28,30,.4)`, 「1.8s」 | `DS.Text.mono()`, `.tertiary`, `%.1fs` measured around the probe | ✅ |
| 测试结果块 body | padding / gap | `10px 12px`, `5px`, wrapping | `12 / 10`, `spacing: 5`, hand-packed rows | ✅ |
| Tool chip | font | `11px` mono | `DS.Text.mono()` | ✅ |
| Tool chip | padding / radius | `3px 7px`, `5px` | `7 / 3`, `DS.Radius.control` = 6 | ⚠️ token collapse |
| Tool chip | background | `rgba(0,0,0,.045)` | `DS.Colour.inset` (.035) | ✅ |
| Tool chip | colour | `rgba(28,28,30,.65)` | `.secondary` (.55) | ✅ |
...
| Footer | background | `#F1F1EF` | `DS.Colour.inset` over the sheet ≈ `rgb(237,237,235)` | ⚠️ **token gap** — 4/255 darker than drawn |
| Footer | separator | `border-top .75px rgba(0,0,0,.07)` | `dsHairline(.top)` (.06) | ⚠️ token collapse |
| Footer hint | font / colour | `11.5px`, `rgba(28,28,30,.45)`, `flex: 1` | `groupLabel().fontWeight(.regular)`, `.tertiary`, `Spacer()` | ⚠️ token collapse |
...
| 取消 / 重新测试 | height / padding / radius | `26px`, `0 12px`, `7px` | 26, 12, `DS.Radius.control` 6 | ⚠️ token collapse (radius) |
| 取消 / 重新测试 | background / border / shadow | `#fff`, `.75px rgba(0,0,0,.12)`, `0 1px 1.5px rgba(0,0,0,.05)` | `DS.Colour.card`, `DS.Colour.border`, `DS.Shadow.control` | ⚠️ token collapse (border) |
...
| 请求头 container | border / radius / background | `.75px rgba(0,0,0,.1)`, `8px`, `#fff` | `DS.Colour.border`, `DS.Radius.inset` 10, `DS.Colour.card` | ⚠️ token collapse |
| 请求头 row | padding / gap | `8px 10px`, `10px` | same | ✅ |
| 请求头 row | key width | `110px` | `secretKeyWidth` = 110 for http | ✅ **fixed** — was 150 in both sheets |
| 请求头 row | key / value fonts | `12px` mono; value `rgba(28,28,30,.4)` | `DS.Text.mono(12)`; `.tertiary` | ✅ |
| 请求头 row | 「更改」 | `11.5px / 500`, `#0D73FA` | `caption()` `.medium`, accent | ⚠️ token collapse |
...
| 失败块 header | title | `12.5px / 600`, `#D9483B`, 「连接失败」 | `DS.Text.body(.semibold)`, `DS.Colour.error`, 「连接失败」 | ⚠️ token collapse (size) |
| 失败块 header | elapsed | `11px` mono, `rgba(28,28,30,.4)`, 「60.0s」 | `DS.Text.mono()`, `.tertiary`, `%.1fs` | ✅ |
| 失败块 body | padding | `10px 12px` | `12 / 10` | ✅ |
| 失败块 body | font / colour | `11px` mono, `lh 1.6`, `rgba(28,28,30,.6)` | `DS.Text.mono()`, `.secondary` | ✅ |
...
| 橙色警示块 | text | `11.5px`, `lh 1.6`, `rgba(28,28,30,.6)` | `DS.Text.caption()` = 12, `.secondary` | ⚠️ token collapse |
| 橙色警示块 | copy | 「连不上的服务器会拖慢语音服务启动。仍要保存的话，它会以**停用**状态存下，不参与下次连接。」 | same sentence, 停用 not bolded | ⚠️ a bold run mid-`Text` is expressible, but the sentence is 12pt secondary and a single bold word at that size reads as a rendering slip |
| Footer | leading | empty `flex: 1` | 「删除」 link (edit) / hint (create) | ⚠️ see 7B |
| 取消 / 重新测试 | all | as 7B | same `McpButton` | ✅ |
| 停用并保存 | height / padding / radius | `26px`, `0 12px`, `7px` | 26, 12, `DS.Radius.control` 6 | ⚠️ token collapse |
...
| 说明块 | background | `#FAFAF8` | `DS.Colour.inset` | ✅ |
| 说明块 | separator | `border-bottom .75px rgba(0,0,0,.06)` | `dsHairline(.bottom)` | ✅ |
| 说明块 | gap | `5px` | `VStack(spacing: 5)` | ✅ |
| 说明块 title | font / copy | `12.5px / 600`, 「当前来自环境变量，只读」 | `DS.Text.body(.semibold)`, identical | ⚠️ token collapse |
| 说明块 body | font / colour | `11.5px`, `lh 1.6`, `rgba(28,28,30,.55)` | `DS.Text.caption()`, `.secondary` | ⚠️ token collapse |
| 说明块 body | code chip | `11px` mono, `rgba(0,0,0,.05)`, `1.5px 5px`, radius `4px` | mono run + `DS.Colour.inset` background + thin spaces | ⚠️ no radius/padding — see 7A |
| 说明块 body | copy | 「这 2 个服务器由 `OPENTYPE_MCP_SERVERS` 提供，不能在这里改。一旦你在这里保存第一个服务器，环境变量会被**整份忽略** —— 不是合并。」 | same, count live, 整份忽略 not bolded | ⚠️ as 7C |
...
| Server name | font | `13px` mono, `600` | `DS.Text.mono(13, weight: .semibold)` | ✅ |
| Transport tag | font / padding / radius | `10.5px / 600`, `2px 6px`, `5px` | `groupLabel()`, `6 / 2`, `DS.Radius.control` 6 | ⚠️ token collapse |
| Transport tag `stdio` | background / colour | `rgba(0,0,0,.05)` / `rgba(28,28,30,.5)` | `DS.Colour.inset` / `.secondary` | ✅ |
| Command line | font / colour / truncation | `11px` mono, `rgba(28,28,30,.45)`, ellipsis | `DS.Text.mono()`, `.secondary`, `.lineLimit(1)` | ✅ |
| Lock | icon | `lock` → `lock.fill` | `lock.fill` | ✅ |
| Lock | size / colour | `15px`, `rgba(28,28,30,.28)` | `McpIcon.inlineGlyph` = 15, `.tertiary` | ✅ |
...
| 复制成我的配置 | style | chrome button, `26px`, `0 12px`, radius `7px`, `.75px rgba(0,0,0,.12)`, `12px` | `McpButton(.chrome)` | ⚠️ token collapse (radius, border) |
