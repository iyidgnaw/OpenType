...
状态：**已完成**（2026-08-14）— T1–T9 全部落地，分支 `feat/dsh-borrowings`。
实际偏离见 §15。
...
| 项 | 偏离 | 为什么 |
|---|---|---|
| T8 | 文档落成一页 `docs/model-context-inventory.md`，源码里只放指针，不是每处写一节 | 一个事实一个家。分散写会立刻开始各自漂移，正是这条规范要解决的问题 |
| T9 | 渲染逻辑放 `src/agent/toolCatalog.ts`，脚本只做 CLI | 让渲染进入被测试的目录树；脚本本身不含逻辑 |
| T2 | `clampToolResult` 从 `loop.ts` 移到 `spill.ts`，`loop.ts` 再导出 | 原方案会造成 `loop ⇄ spill` 循环导入。截断现在是 spill 的**回退分支**，不是独立策略，所以归属它更正确 |
| T2 | 文件名消毒的白名单不含 `.` | 点本身不能穿越（分隔符已全部折叠），但会让 `..` 留在路径里被运维 grep 或粘进命令。扩展名由本模块添加，调用方的点毫无用处 |
| T1 | 取消入口做了**两个**（面板 + Agent tab 行），不是一个 | 语音面板是临时的：一开口、切模式、或新 dispatch 都会让它消失，而那恰好是长任务最需要停止的时刻；且它够不到非最新的 run |
| T1 | 499 单独作判别符，不加 `code` 字段 | 路由只渲染 `{ error: message }`，多一个字段是**读起来像机器可读、实际不可达**的死重量 |
| T6 | 审批结果不再携带策略自定义的自由文本 | 自由文本理由移到了 `ToolGuard`——那才是真正有具体理由可说的一层。审批结果回答"可否放行"，不负责解释自己 |
| T6 | 审计对的 sink 留空未接 run log | 审批对属于某个 run，而只有 agent route 知道调用属于哪个 run；接线留给需要它的第一个真实策略 |
| T7 | 持久化归 sidecar 而非 Swift 的 `ImmutableAuditStore` | 那个 store 是**按请求**粒度的，而步骤发生在 sidecar；为持久化把每一步搬过 socket 是本末倒置 |
| T5 | `ask_user` 即使无 UI 通道也保持在工具目录里 | 工具目录随请求变形会破坏可复用前缀。稳定目录 + 诚实拒绝，与 plan mode 保留 `exit_plan_mode` 是同一个取舍 |
| T5 | 问题复用现有 ~0.7s 进度轮询，不新开循环 | "在等什么"本来就是"在做什么"的一部分 |
