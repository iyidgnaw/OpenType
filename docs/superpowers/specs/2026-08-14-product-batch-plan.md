...
**产品意图**：把 Review 从「全局开关」变成「事后可选」。直接模式照常立刻交付，不增加任何延迟；但刚交付的
那一条在 **8 秒**内保持「热」，窗口内按热键 = 对它做语音纠错，而不是开始新录音。

- 常量 `correctionWindowSeconds = 8`。选 8 秒的理由：足够看一眼刚粘进去的文字并反应过来，又短到不会让人
  「想说下一句却被抢走热键」。这是本批唯一一个凭直觉定的数，明确标注为待调。
- 纯逻辑座 `CorrectionWindow`（可单测）：`isHot(now, deliveredAt) -> Bool`，以及热键按下时的判定
  `HotKeyIntent.correctLastResult / startNewRecording`。
- `AppModel.hotKeyPressed()` 的判定顺序变成：Review 面板开着 → 面板纠错；否则窗口热 → 打开 Review 面板并
  **预填刚交付的文本**，进入纠错流程；否则 → 新录音。
- 视觉：交付成功的 toast 在窗口期内保持可见（不是 0.9s 就消失），并显示「再按一次可以纠正」+ 一个走完就消失的
  进度条。**用户能看见这个窗口存在，否则这个功能等于不存在。**
- 窗口在以下情况立即失效：用户开始新录音、按 Esc、前台应用改变、窗口自然到期。
- 只对 `transcribe` + Direct 生效。ask/助理 的卡片已经有自己的「卡片还在就继续会话」语义（`eaa03f3`），
  两者不能抢同一个热键。

**与 P0-2 的连接**：走这条路的纠错同样入词典。这才是闭环真正闭上的那一下——用户改一次，下次就对了。
