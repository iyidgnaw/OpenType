...
`excludeModes` 默认 `[]`，即三个模式全带。**这是 §六 那笔隐私欠账的唯一收口点**：
将来加时间窗或用户开关，改的是这一个参数和它的调用方，不是回头翻整条链路。
参数的 doc comment 必须写明它现在为什么是空的。
...
听写是唯一一个「对着别的 app 说话」的模式，最近 10 条里可能有在微信里口述的私人消息。
`episodic_events` 存的是全文，不是摘要。本设计**不解决**这个问题，产品负责人已明确
将其延后。

记账方式不是当它不存在，而是留一个明确的收口点：`recentEvents` 的 `excludeModes`
参数（§3.4）。将来的方案——时间窗、用户开关、或对听写行做摘要——都在这一个点上落地。
