...
⚠️ **终止边界，2026-08-28 owner 订正（原文有安全缺陷，见下）**：**`startDir` 必须落在 `homeDir` 之内（含 `homeDir` 本身）——不在其内的，直接返回"未找到"，一步都不向上走**。判断"是否在内"要先把 `startDir`、`homeDir` 都过一遍 `realpath`（展开符号链接）再比较，不能是裸字符串比较。`homeDir` 本身仍是含边界：`AGENTS.md` 直接放在 `homeDir` 下会被找到。

这条边界最初写的是「用户家目录（含）或文件系统根，以先到者为准」，字面实现（`sidecar/src/agent/projectAgentsMd.ts` 第一版，stage-4 review 发现）会在 `startDir` 与 `homeDir` 没有共同祖先时——例如 `workingDirectory` 指向 `/tmp` 下某目录，或一条解析后落在 home 之外的符号链接路径——一路走到文件系统根，沿途读取任何祖先目录里同名的 `AGENTS.md`，包括 `/tmp/AGENTS.md`、`/AGENTS.md`。macOS 上 `/tmp` 全局可写，任何本地进程都能在那里放一份文件；这个 agent 默认没有沙箱、没有审批确认（§2.1），会直接把这样一份文件当"项目约定"读进去执行。这不是可以容忍的边界情况，是一条现成的攻击路径——旧文本本身就是缺陷来源，不只是某次实现漏看了它。

裸字符串比较同样不够：`path.resolve` 只处理 `.`/`..`/末尾斜杠，不展开符号链接，而 macOS 常见路径（`/tmp`、`/var` 等，也是 `os.tmpdir()` 的所在）本身就是符号链接目标，字符串比较会被这类路径悄悄绕过（`homeDir` 与 `startDir` 实际指向同一目录，却因拼法不同永远比不相等，导致边界形同虚设）。
