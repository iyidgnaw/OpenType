...
（`ASK_TOOL_NAMES` 因此必须导出并被测试直接引用：这条白名单是「历史读取工具永不到达 ask」
这个保证的全部依据，测试里放一份手抄副本等于什么都没证明。）

**`limit` 必须夹紧，这是本设计新造出来的暴露面。** SQLite 的 `LIMIT` 遇到负数是「不限制」，
所以 `{limit: -1}` 会把整张 `episodic_events` 未截断地倒进 agent 上下文。
在本批之前那只是烧 token；本批之后那张表装着**用户的全部听写内容**，
于是一次幻觉或被诱导的工具调用就能读回全部口述历史。
契约：非正整数回退到默认值，超过 50 夹到 50。

**这条规则对 `episodic_events` 的每一条读路径都成立，不只是这个工具。**
本批里同一个 bug 出现了两次：`opentype__read_history` 的 `limit`，以及
`GET /memory/events?limit=`。根因是 SQLite 把负数 `LIMIT` 当作「不限制」，
而本批恰好给「一张装着用户全部口述内容的表」接上了多个带 limit 的读接口。
**以后每新增一条读这张表的路径，夹紧就要再做一次**——这不是某个消费者的实现细节，
是这张表的准入条件。两处的上限可以不同（工具的上限约束进入模型上下文的量，
端点的上限约束用户滚动的列表长度），但「非正整数回退默认值、超上限夹到上限」这个形状必须一致。

更一般地说，这个暴露面是两个各自合理的决定叠加出来的——
「三个模式统一进一个存储」加上「给 agent 一个按 id 读它的工具」。
夹紧 `limit` 堵住的是最粗的口子，不是完整答案：
agent 仍然可以分多次调用逐段读回历史。这与 §六 的隐私欠账是同一件事的两面，
而且**优先级更高**——时间窗管的是「每次注入多少」，这里管的是「一次能捞出多少」。
...
**就地纠错的交付不更新记忆。** `processInPlaceCorrection`（P0-3，交付后 8 秒内的纠错窗口）
会把一段新的最终文本送进目标应用，并写一条 `.corrected` 审计事件——但不写 episodic。
那一轮的记忆记录停留在 `.completed` 时捕获的原文。

不在本批修，因为这与被删掉的 `HistoryStore` 行为一致（它同样从不因就地纠错更新），
所以不是本批引入的回归。但值得记下来：**用户看到的最终文字与记忆里存的可以不一致**，
而这个差异恰好发生在用户主动纠正过的地方——也就是最该被记住的那种修改。
Review 模式没有这个问题（它在提交时才写，纠错轮次都在提交之前）。
