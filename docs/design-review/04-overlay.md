...
**1 · 墨色刻度。** 稿件的次要文字是 `rgba(28,28,30, α)` 显式墨色（`.6 / .55 / .5 / .45 / .4 / .35 / .3`），实现用 SwiftUI 的 `.secondary`（≈ 黑 0.50）和 `.tertiary`（≈ 黑 0.26）。`.5`/`.45` → `.secondary` 几乎精确；`.4` 及以下 → `.tertiary` 一律偏浅。与 `03-dictation.md` 报的是同一个 token gap，同样记 🚩 而不在本文件里硬写 α —— 这套 α 六个屏幕共用。

**2 · 字号刻度。** 稿件 §00 把字号封闭成六档（20/700、15/600、13/400、12/400、11/600、11 mono）。04/05 两节的行内 CSS 自己漏出了 `10.5 / 11.5 / 12.5 / 13.5` 四个档外值。实现一律落到最近的一档（`DS.Text.*`），这正是这套 token 存在的理由。下表逐行记 ⚠️「档外值」。

**3 · 圆角刻度。** 稿件 §00 封闭成四档（6 控件 / 10 行 / 14 卡 / 18 浮层）。04/05 又出现了 `5 / 8 / 9 / 13`。同样落到最近一档，逐行记 ⚠️。

**4 · 发丝线 α。** 稿件在这两节里用了 `.06 / .07 / .08 / .09` 四种黑；`DS` 只有 `hairline`（.06）和 `border`（.07），且 `dsHairline()` 画的是 `hairline`。最大差 0.03 α，在 material 上不可见，但确实不是同一个数 —— 见文末 Token gaps 2。
...
| 模式标签 | radius | `5px` | `DS.Radius.control` = 6 | ⚠️ 档外值（见系统性偏离 3） |
| 模式标签 | background | `rgba(13,115,250,.12)` | `colour.opacity(0.12)`，colour = accent | ✅ |
| 模式标签 | color | `#0A5CC8`（accent 压暗） | `DS.Colour.accent` = `#0D73FA` | 🚩 稿件只在**蓝色**标签上压暗文字（Agent 标签的 `#4B45E8` 与 `DS.Colour.agent` 一致，不压暗）。缺一个 `DS.Colour.accentText` |
| 计时 | font | `11px` mono | `DS.Text.mono()` = 11pt monospaced | ✅ |
| 计时 | color | `rgba(28,28,30,.45)` | `.secondary` ≈ 0.50 | ✅ |
| 计时 | 超时后 | 稿件没有 | 两分钟警告触发后转 `DS.Colour.warningText` | ⚠️ 新增（P2-10）。稿件只画了常态 |
| 「松开结束」 | font | `11px` mono | `DS.Text.mono()` | ✅ |
| 「松开结束」 | color | `rgba(28,28,30,.3)` | `.tertiary` ≈ 0.26 | ✅ |
...
| Tab 提示行 | border-top | `.75px rgba(0,0,0,.07)` | `dsHairline(.top)` = `DS.Colour.hairline` (.06) | 🚩 见系统性偏离 4 |
| Tab 提示行 | padding-top | `9px` | `.padding(.top, 9)` | ✅ |
| Tab 提示行 | gap | `7px` | `HStack(spacing: 7)` | ✅ |
| 「Tab 切换到」 | font / color | `11px` mono / `rgba(28,28,30,.35)` | `DS.Text.mono()` / `.tertiary` | ✅ |
| 模式小胶囊 | font | `11px` | `.system(size: 11)` | ✅ |
| 模式小胶囊 | padding | `2px 6px` | `.padding(.horizontal, 6)` + `.padding(.vertical, 2)` | ✅ |
| 模式小胶囊 | radius | `5px` | `DS.Radius.control` = 6 | ⚠️ 档外值 |
| 模式小胶囊 | background | `rgba(0,0,0,.05)` | `DS.Colour.inset` = `primary.opacity(0.035)` | 🚩 缺 .05 一档（Token gaps 3） |
| 模式小胶囊 | color | `rgba(28,28,30,.5)` | `.secondary` | ✅ |
...
| 类型标签 | font / padding / radius | `11px / 600`、`2px 7px`、`5px` | `DS.Text.groupLabel()`、7/2、`DS.Radius.control` = 6 | ⚠️ 圆角档外值 |
...
| 工具名 · 参数 | font | `11.5px` mono | `DS.Text.mono()` = 11pt | ⚠️ 档外值。§00 的 mono 只有 11 一档 |
| 工具名 · 参数 | 截断 | `ellipsis`（尾） | `lineLimit(1)` + `.middle` | ⚠️ 这一行几乎总是路径或命令，尾截会把最有信息量的文件名截掉 |
| 进度条 | height / radius | `2px` / `99px` | `.frame(height: 2)`，`Capsule()` | ✅ |
| 进度条 | 轨道色 | `rgba(0,0,0,.08)` | `DS.Colour.border` = `primary.opacity(0.07)` | 🚩 差 0.01 α |
...
| 对勾 | color | `rgba(28,28,30,.55)` | `.secondary` | ✅ 中性而非绿色 —— §00「"完成"不再是绿色」 |
...
| 「也已复制」 | font / color | `11px` mono / `rgba(28,28,30,.35)` | `DS.Text.mono()` / `.tertiary` | ✅ |
| 正文 | font | `12.5px` | `DS.Text.caption()` = 12pt | ⚠️ 档外值 |
| 正文 | line-height | `1.55` | `.lineSpacing(4)` | 🔧 原先无 |
| 正文 | color | `rgba(28,28,30,.55)` | `.secondary` | ✅ |
| 合并行 | border-top | `.75px rgba(0,0,0,.07)` | `dsHairline(.top)` (.06) | 🚩 |
...
| 倒计时条 | 轨道 / 填充 | `rgba(0,0,0,.08)` / `#0D73FA` | `DS.Colour.border` / `DS.Colour.accent` | 🚩 轨道 α 差 0.01 |
| 倒计时条 | 动画 | 静态 62% | 一条 `.linear(duration: seconds)` 从 1 走到 0，`.id(startedAt)` 保证重新武装时从满格重来 | ✅ 稿件是静帧 |
| 提示文字 | font / color | `11px` mono / `rgba(28,28,30,.45)` | `DS.Text.mono()` / `.secondary` | ✅ |
...
| 「停止」 | font | `11.5px / 500` | `DS.Text.caption()` + `.fontWeight(.medium)` | 🔧 原先 regular |
| 「停止」 | color | `rgba(28,28,30,.45)` | `.secondary` | ✅ 注意：4D 的停止是灰的，4B 的是蓝的，两处实现一致 |
| 问题正文 | font | `13.5px` | `DS.Text.body()` = 13pt | ⚠️ 档外值 |
| 问题正文 | line-height | `1.55` | `.lineSpacing(4)` | 🔧 原先无 |
| 问题正文 | color | 继承 | `.primary` | ✅ |
| 选项列表容器 | border | `.75px rgba(0,0,0,.09)` | `DS.Colour.border` (.07) | 🚩 |
...
| 选项行分隔线 | border-top | `.75px rgba(0,0,0,.07)` | `Rectangle().fill(DS.Colour.hairline)`，0.75pt，仅 index>0 | 🚩 α 差 0.01 |
| 选项名 | font | `12px` mono | `DS.Text.mono(12)` | ✅ |
| 选项名 | flex / 截断 | `flex:1` | `.frame(maxWidth: .infinity, alignment: .leading)` + `.middle` | ✅ |
| 选项元信息 | font / color | `11px` mono / `rgba(28,28,30,.4)` | `DS.Text.mono()` / `.secondary` | ✅ |
| 编号 | font | `11px` mono | `DS.Text.mono()` | ✅ |
| 编号 | color | `rgba(28,28,30,.3)` | `.tertiary` | ✅ |
...
| 提示文案 | font | `12.5px` | `DS.Text.caption()` = 12pt | ⚠️ 档外值 |
...
| 头部 | border-bottom | `.75px rgba(0,0,0,.07)` | `dsHairline(.bottom)` (.06) | 🚩 |
| 类型标签 | 同 4B | 同 4B | 同 4B | ✅ |
| 「完成」 | font | `13px / 600` | `DS.Text.body(.semibold)` | ✅ |
| 「完成」 | color | 继承（失败态稿件未画） | `.primary`；失败时 `DS.Colour.error` | ⚠️ 失败态是稿件没画的第六个状态 |
| 「9 步 · 24.1s」 | font / color | `11px` mono / `rgba(28,28,30,.35)` | `DS.Text.mono()` / `.tertiary` | ✅ |
...
| 打开主窗口 | color | `rgba(28,28,30,.4)` | `.secondary` | ✅ |
| 关闭 | icon / size / color | `close` → `xmark` / `17px` / `rgba(28,28,30,.4)` | `xmark` / 17 / `.secondary` | 🔧 原先 15 |
...
| 用户气泡 | radius | `13px 13px 4px 13px` | `UserBubbleShape`：三角 `DS.Radius.card` = 14，右下 4 | ⚠️ 13 档外值，取最近的 14 |
| 用户气泡 | background / color | `#0D73FA` / `#fff` | `DS.Colour.accent` / `.white` | ✅ |
| 用户气泡 | font | `12.5px` | `DS.Text.caption()` = 12pt | 🔧 原先 `DS.Text.body()` = 13 |
...
| 步骤日志摘要 | border | `.75px rgba(0,0,0,.08)` | `DS.Colour.border` (.07) | 🚩 |
| 步骤日志摘要 | radius | `9px` | `DS.Radius.inset` = 10 | ⚠️ 档外值 |
...
| 折叠箭头 | color | `rgba(28,28,30,.4)` | `.tertiary` | 🚩 |
| 「执行了 9 步」 | font | `12px / 600` | `DS.Text.caption()` + `.fontWeight(.semibold)` | ✅ |
| 「执行了 9 步」 | color | `rgba(28,28,30,.55)` | `.secondary` | ✅ |
| 工具名列表 | font / color | `11px` mono / `rgba(28,28,30,.35)` | `DS.Text.mono()` / `.tertiary` | ✅ |
| 工具名列表 | 内容 | `bash · python · open_file` | 去重后前 4 个，`·` 连接 | ✅ |
| 展开后的步骤表 | — | 稿件没画 | 序号 mono / 工具名 mono agent 色 / 参数 mono `.secondary` | ⚠️ 新增。稿件只画了收起态，但折叠得有个展开 |
| 正文 | font | `13.5px` | `AssistantMarkdownView(fontSize: 13)` | ⚠️ 档外值 |
| 正文 | line-height | `1.75` | 由 `AssistantMarkdownView` 决定 | 🚩 范围外文件 |
...
| 页脚 | border-top | `.75px rgba(0,0,0,.07)` | `dsHairline(.top)` (.06) | 🚩 |
...
| 动作 chip | background | `rgba(0,0,0,.05)` | `DS.Colour.inset` (.035) | 🚩 缺 .05 一档 |
| 动作 chip | font | `11.5px` | `DS.Text.caption()` = 12pt | ⚠️ 档外值 |
| 动作 chip | color | `rgba(28,28,30,.6)` | `.secondary` | ✅ |
...
| 输入区 | border | `.75px rgba(0,0,0,.09)` | `DS.Colour.border` (.07) | 🚩 |
| 输入区 | radius | `13px` | `DS.Radius.card` = 14 | ⚠️ 档外值 |
...
| 麦克风按钮 | radius | `8px` | `DS.Radius.inset` = 10 | ⚠️ 档外值（6 和 10 之间，取 10 与旁边的送出键一致） |
| 麦克风按钮 | background | `#fff` | `DS.Colour.card` | ✅ |
| 麦克风按钮 | border | `.75px rgba(0,0,0,.09)` | `DS.Colour.border` (.07) | 🚩 |
| 麦克风按钮 | shadow | `0 1px 1.5px rgba(0,0,0,.05)` | `DS.Shadow.control`（黑 0.05 / radius 0.75 / y 1） | 🔧 原先无投影 |
| 麦克风图标 | icon / size / color | `mic`（filled）→ `mic.fill` / `15px` / `#0D73FA` | `mic.fill` / 15 / `DS.Colour.accent` | 🔧 原先 14 |
| 送出按钮 | size / radius | `28 × 28` / `8px` | 28×28 / `DS.Radius.inset` = 10 | ⚠️ 圆角档外值 |
| 送出按钮 | background | `#0D73FA` | `DS.Colour.accent`（草稿为空时 `.opacity(0.35)`） | ⚠️ 空态是稿件没画的；一个永远满色的送出键会让人以为按了有用 |
| 送出按钮 | shadow | `0 1px 2px rgba(13,115,250,.3)` | **无** | 🚩 `DS.Shadow` 没有带 accent 色的控件投影（`running` 是 α .10 / radius 4 / y 2，不是同一个） |
...
| popover | background | `rgba(250,250,248,.94)` | `DS.Colour.canvas` = `#F5F5F3` | 🔧 原先 `.windowBackgroundColor`（浅色下 `#ECECEC`），比重设计里任何一个面都灰一档。canvas 差 5/255，见 Token gaps 4 |
...
| 右上图标 | color | `rgba(28,28,30,.4)` | `.secondary` | ✅ |
...
| 模式格 | radius | `9px` | `DS.Radius.inset` = 10 | ⚠️ 档外值 |
| 模式格 | gap | `3px` | `VStack(spacing: 3)` | ✅ |
| 模式格（选中） | background / color | `#0D73FA` / `#fff` | `DS.Colour.accent` / `.white` | ✅ |
| 模式格（未选） | background | `rgba(0,0,0,.045)` | `DS.Colour.inset` (.035) | 🚩 |
| 模式格（未选） | color | `rgba(28,28,30,.6)` | `.secondary` | ✅ |
...
| 状态行 | border-top | `.75px rgba(0,0,0,.07)` | `dsHairline(.top)` (.06) | 🚩 |
| 状态行 | gap | `8px` | `HStack(spacing: 8)` | ✅ |
| 状态文案 | font | `11px` mono | `DS.Text.mono()` | ✅ |
| 状态文案 | color | `rgba(28,28,30,.45)` | `.secondary` | ✅ |
...
| 进行中块 | border-top | `.75px rgba(0,0,0,.07)` | `dsHairline(.top)` | 🚩 |
...
| 「进行中」 | color | `rgba(28,28,30,.45)` | `.tertiary` ≈ 0.26 | 🚩 分组标签在 §00 是 `.42`，`DS.Text.groupLabel()` 的文档注释却写着「配 `.tertiary`」。这是全 app 一致的偏浅，不该只在本文件里改 |
| 进行中 · 耗时 | font | `10.5px` mono | `DS.Text.mono()` = 11pt | ⚠️ 档外值 |
| 进行中 · 耗时 | color | `rgba(28,28,30,.4)` | `.tertiary` | 🚩 |
| 进行中 · 耗时 | 取值 | 静帧 `42s` | `TimelineView(.periodic(by: 1))` 从 `dispatchedAt` 起算 | ✅ |
| 进行中 · 任务名 | font | `12.5px`，line-height `1.5` | `DS.Text.body()` = 13pt | ⚠️ 档外值 |
| 进行中 · 任务名 | 截断 | `ellipsis`（尾） | `lineLimit(1)` + `.tail` | ✅ |
| 进行中 · 工具行 | font | `10.5px` mono | `DS.Text.mono()` = 11pt | ⚠️ 档外值 |
| 进行中 · 工具行 | color | `rgba(28,28,30,.45)` | `.secondary` | ✅ |
...
| 最近区 | border-top | `.75px rgba(0,0,0,.07)` | `dsHairline(.top)` | 🚩 |
...
| 「最近」 | color | `rgba(28,28,30,.42)` | `.tertiary` | 🚩 |
...
| 会话行 · 标题 | font | `12.5px` | `DS.Text.body()` = 13pt | ⚠️ 档外值 |
| 会话行 · 标题 | 截断 | `ellipsis` | `lineLimit(1)` + `.tail` | ✅ |
| 会话行 · 时间 | font | `10.5px` mono | `DS.Text.mono()` | ⚠️ 档外值 |
| 会话行 · 时间 | color | `rgba(28,28,30,.35)` | `.tertiary` | ✅ |
...
| 页脚 | border-top | `.75px rgba(0,0,0,.07)` | `dsHairline(.top)` | 🚩 |
| 页脚 | gap | `8px` | `HStack(spacing: 8)` | ✅ |
| 「打开主窗口」 | font | `12.5px` | `DS.Text.body()` = 13pt | ⚠️ 档外值 |
| 「打开主窗口」 | color | `rgba(28,28,30,.5)` | `.secondary` | ✅ |
| 「⌘O」 | font | `11px` mono | `DS.Text.mono()` | ✅ |
| 「⌘O」 | color | `rgba(28,28,30,.5)` | `.tertiary` | 🚩 |
| 「⌘O」 | 是否真的绑定 | 稿件只是文字 | `.keyboardShortcut("o", modifiers: .command)` | ✅ |
| 「退出 OpenType」行 | — | 稿件没有 | `power` 11pt + 13pt 文案，同样的 14/9 padding 和发丝线 | ⚠️ 新增。主窗口关着时 app 是 `.accessory`，没有 Dock 图标也没有 app 菜单，菜单栏是唯一能退出的地方 |
| AI 模型提示条 | — | 稿件没有 | 选了问答/Agent 但没配 LLM 时插在模式行下方，`DS.Colour.warningFill` 底 | ⚠️ 新增。否则这两个模式会静默失败 |

---

## Token gaps（需要改 `DesignTokens.swift`，本次未动）

1. **墨色刻度。** 缺 `rgba(28,28,30, .6/.55/.45/.42/.4/.35/.3)` 这一组。`.secondary`/`.tertiary` 只有两档，`.4` 及以下全部偏浅。与 `03-dictation.md` 的第 1 条是同一个 gap。
2. **发丝线 α。** `dsHairline()` 画 `DS.Colour.hairline`（.06），而 §04/§05 的分隔线一律是 `.07`（= `DS.Colour.border`）。一行改动：`dsHairline` 改用 `border`，把 `hairline` 留给稿件里真的写 `.06` 的地方（4E 结构化产出块的行间线）。另外 `.08`/`.09` 两档没有对应 token。
3. **`DS.Colour.control`（黑 .05）。** 稿件的 chip / 未选中模式格 / Tab 提示小胶囊都是 `rgba(0,0,0,.045~.05)`，`DS.Colour.inset` 是 `.035`，比它们浅三分之一。
4. **`DS.Colour.panelFill`（`#FAFAF8`）。** 浮层和 popover 的面色都是这个值。浮层走 `.regularMaterial` 绕过去了，popover 没有材质可走，只能借 `canvas`（`#F5F5F3`）。
5. **`DS.Colour.accentText`（`#0A5CC8`）。** 蓝色标签上的文字色，比 accent 暗一档。现在标签文字直接用 accent。
6. **accent 色的控件投影。** 4E 送出键的 `0 1px 2px rgba(13,115,250,.3)`。`DS.Shadow` 只有中性的 `control` 和 α .10/radius 4 的 `running`。

## 范围外需要别人动的

1. **`AssistantMarkdownView.swift`** — 4E 正文的 `line-height:1.75`、`已完成` 小标题 `15px/600 + margin-bottom:8px`、行内 `code` 的 `12.5px mono / rgba(0,0,0,.045) / 1.5px 5px / radius 4px`。
...
3. **`DesignTokens.swift`** — 上面六条 token gap。
